...
namespace MvcWebRole.Controllers
{
    public class FundController : ApiController
    {
        ...
        public HttpResponseMessage Post([FromBody]Fund fund)
        {
            ...
            fund.FiscalYear = Int16.Parse(WebConfigurationManager.AppSettings["FiscalYear"]);
        ...
        [Authorize(Roles = "MANAGE-FUNDS")]
        [HttpPatch]
        public HttpResponseMessage RollForward()
        {
            foreach (var fund in repository)
            {
                fund.CurrentBudget = fund.CurrentBudget + fund.BudgetAdjustment;
                fund.BudgetAdjustment = 0;
                fund.ProjectedExpenditures = 0;
                fund.Status = Status.Draft;
                fund.BudgetAdjustmentNote = String.Empty;
                fund.FileUploads = null;
                fund.FiscalYear = Int16.Parse(WebConfigurationManager.AppSettings["FiscalYear"]);
